...
def load_video(video_path, max_frames_num=256,fps=1,force_sample=False):
    try:
        ...
        spare_frames = np.zeros((256, 336, 336, 3)).astype(np.uint8)
...
class LLaVAVideoGRPOTrainer(Trainer):
    """
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            if "Qwen2-VL" in model_id:
                ...
                # processing_class, model, image_processor, max_length = load_pretrained_model(model_id,  
                #     None, "llava_qwen", torch_dtype="bfloat16", 
        ...
        if is_deepspeed_zero3_enabled():
            if "Qwen2-VL" in model_id:
                ...
                self.ref_model = LlavaQwenForCausalLM.from_pretrained(model_id, low_cpu_mem_usage=False, attn_implementation=attn_implementation, torch_dtype="bfloat16")
            else:
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if 'dvd' in self.video_folder:
            for idx in range(len(inputs)):
                inputs[idx]['video'] = inputs[idx].pop("video_filename")
        ...
        for kk in range(len(video_inputs)):
            video_inputs[kk] = self.image_processor.preprocess(video_inputs[kk], return_tensors="pt")["pixel_values"].to(device).to(torch.bfloat16)  
        ...
        def get_per_token_logps(model, input_ids, completion_length=None, images=None):
            # import pdb; pdb.set_trace()
            logits = []
            for i in range(len(input_ids)):
                logits.append(model(input_ids[i:i+1], images=images, modalities=["video"]).logits)  # (B, L, V)
        ...
        per_token_logps = get_per_token_logps(model, prompt_completion_ids, 
                                              images=video_inputs, completion_length=max_length,
                                             )
        ...
        with torch.inference_mode():
            if self.ref_model is not None:
                ref_per_token_logps = get_per_token_logps(self.ref_model, prompt_completion_ids, images=video_inputs, completion_length=max_length)
            else:
                with self.accelerator.unwrap_model(model).disable_adapter():
                    ref_per_token_logps = get_per_token_logps(model, prompt_completion_ids, images=video_inputs, completion_length=max_length)
